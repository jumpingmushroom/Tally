...
namespace Tally.Net
{
    ...
    ///
    /// Two methods. "Tally_Hello" announces presence, so an attacker knows whether the owner
    /// of what it hits will report real damage. "Tally_Events" carries a batch of events as a
    ...
    public static class EventSync
    {
        ...
        private static void Register(ZRoutedRpc rpc)
        {
            ...
            if (PluginConfig.Verbose.Value)
                TallyPlugin.Log.LogDebug("routed RPC handlers registered, nonce " + _nonce);
        }
        ...
        private static void SendHello(long target)
        {
            ...
            pkg.Write(Protocol);
            pkg.Write(_nonce);
            pkg.Write(TallyPlugin.PluginVersion);
        ...
        private static void OnHello(long sender, ZPackage pkg)
        {
            try
            {
                ...
                {
                    if (PluginConfig.Verbose.Value)
                        TallyPlugin.Log.LogDebug("peer with Tally: " + name + " (" + sender + ", v" + version + ")");
            ...
            {
                TallyPlugin.Log.LogWarning("bad hello from " + sender + ": " + ex.Message);
            }
        }
        ...
        private static void Flush()
        {
            ...
            while (offset < _outgoing.Count)
            {
                ...
                EventsSent += count;
                if (PluginConfig.Verbose.Value)
                    TallyPlugin.Log.LogDebug("sent " + count + " event(s), " + pkg.Size() + " bytes");
                offset += count;
            }
            _outgoing.Clear();
        }
        ...
        private static void OnEvents(long sender, ZPackage pkg)
        {
            ...
            {
                TallyPlugin.Log.LogWarning("bad event packet from " + sender + ": " + ex.Message);
            }
        }
    }
}
