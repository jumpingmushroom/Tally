...
namespace Tally.Core
{
    ...
    /// Lives in BepInEx/config/Tally/, so it follows the profile.
    /// </summary>
    public static class RecordStore
    {
        ...
        {
            string dir = System.IO.Path.Combine(BepInEx.Paths.ConfigPath, "Tally");
        ...
        private static void Load(string worldName)
        {
            ...
            {
                TallyPlugin.Log.LogInfo("no records yet at " + _path);
                return;
            }
            ...
            try
            {
                ...
                TallyPlugin.Log.LogInfo(string.Format("loaded records for {0} weapon(s) from {1}", _weapons.Count, System.IO.Path.GetFileName(_path)));
            }
            catch (Exception ex)
            {
                TallyPlugin.Log.LogWarning("could not read records " + _path + ": " + ex.Message);
                _weapons.Clear();
                _overall = null;
            }
        }
        ...
        private static void Save()
        {
            ...
            try
            {
                ...
                if (PluginConfig.Verbose.Value)
                    TallyPlugin.Log.LogDebug("saved records to " + System.IO.Path.GetFileName(_path));
            }
            catch (Exception ex)
            {
                TallyPlugin.Log.LogWarning("could not write records " + _path + ": " + ex.Message);
            }
        }
    }
}
